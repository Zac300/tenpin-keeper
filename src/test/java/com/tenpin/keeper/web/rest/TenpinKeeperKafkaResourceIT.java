...
import com.tenpin.keeper.config.KafkaProperties;
...
class TenpinKeeperKafkaResourceIT {
    ...
    void setup() {
        ...
        TenpinKeeperKafkaResource kafkaResource = new TenpinKeeperKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/tenpin-keeper-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/tenpin-keeper-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
